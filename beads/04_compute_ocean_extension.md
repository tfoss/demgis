...
2. **Multi-part input decomposition AND relevance clip before tangent search.** Both `A_geom` and each candidate `B_geom` must be decomposed into connected components whose individual area ≥ `CountryGroup.min_island_area_km2[member]` (per the existing per-member dict at `groups.py:100`) BEFORE being handed to `build_sector_polygon`. Without this, Japan (109 NE parts) produces a 4-vertex degenerate sector — bead 02 verified that feeding Honshu-only + Korea-mainland-only produces the correct 1,614-vertex Tsushima-Strait-tracing polygon. This is also what resolves the Sri Lanka ↔ India hull-overlap case (bead 07): the full NE polygons overlap, but their largest-component sub-polygons do not. The decomposition also handles the UK "continental" classification (bead 08): the GB sub-polygon is an island even though the full UK polygon shares the NI/Ireland border.

   **Decomposition is not sufficient on its own.** Even after decomposing Russia (33 sub-polygons), the largest sub-polygon (Russia-mainland, 16M km²) spans from Kaliningrad to the Pacific. Its nearest point to Japan is within 1,000 km but its far edge is ~10,000 km away, so its convex hull stretches across all of Eurasia and tangent lines from Japan form a sector polygon spanning continents. The fix: **clip each `b_sub` to a relevance disc of radius `max_distance_km` around `A_full`** before tangent search. The clipped sub-polygon contains only the portion of B within range — China's eastern coastal strip rather than the whole country, Russia's far-east Pacific shore rather than mainland-plus-Kaliningrad. Implementation: `clipped = b_sub.intersection(A_full.buffer(max_distance_km * 1000))`, then filter `_components(clipped)` by the same area threshold so the disc-boundary slivers don't pollute the tangent search.
