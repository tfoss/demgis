...
1. Does the group cover only mainland GB, or include Northern Ireland (which would make GB+NI a single landmass for the algorithm and shift the Ireland pair to GB+NI↔ROI)? Default: follow NE's UK polygon (includes NI) and let the algorithm see one combined entity.
