...
Per `MIGRATION_PLAN_DRAFT.md §5c` pilot 2, Sri Lanka is the cleanest *algorithmic* validation point **once bead 04's multi-part decomposition is in place** (this clarification is load-bearing — see below). Pilot 1 (Japan+Korea, bead 06) is the regression test against an existing physical print — it proves the new algorithm reproduces a known-good footprint. Sri Lanka has no such prior; it tests the algorithm on its own merits. Single ownership pair (SL island ↔ India continental → SL owns per `OCEAN_TILE_GUIDELINES.md §Ownership rule`). The Palk Strait is narrow (~30 km), so the sector polygon will be a short, narrow slice from northern SL up to India's southern coast (Tamil Nadu) with Adam's Bridge / Rama Setu shoals appearing as third-party land subtracted via step 3.6. South of SL is open Indian Ocean — buffer halo only.

**Spec correction from bead 02 implementation:** earlier drafts said "convex hulls don't overlap (no Kastellorizo-style degenerate)." That's wrong. In Equal Earth, India's full NE convex hull engulfs Sri Lanka's northern Adam's Bridge area — `find_outer_tangents` correctly raises `HullsOverlapError` when fed the full NE polygons. The Sri Lanka pilot therefore depends on bead 04's deliverable #2 (multi-part decomposition by `min_island_area_km2`): once the algorithm operates on each country's largest sub-polygon, the hulls are disjoint and the sector computes cleanly. If decomposition is missing, this pilot falls back to `override_polygon`. The "simplest algorithmic pilot" framing is correct only POST-decomposition.
