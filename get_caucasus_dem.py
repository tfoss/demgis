#!/usr/bin/env python3
"""
Download SRTM tiles for the Caucasus region (Armenia, Azerbaijan, Georgia).
...
os.makedirs("caucasus_tiles", exist_ok=True)

# Load Caucasus countries to determine bounds
gdf = gpd.read_file('data/ne/ne_10m_admin_0_countries.shp')
caucasus = gdf[gdf['ADMIN'].isin(['Armenia', 'Azerbaijan', 'Georgia'])]

# Get bounds and calculate tile range
bounds = caucasus.total_bounds
min_lon, min_lat = int(bounds[0]), int(bounds[1])
max_lon, max_lat = int(bounds[2]) + 1, int(bounds[3]) + 1

print(f"Caucasus region: {bounds}")
...
for tile_name, url in tiles_to_download:
    output_file = f"caucasus_tiles/{tile_name}.hgt.gz"
