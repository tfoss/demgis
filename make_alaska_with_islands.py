"""
Generate Alaska STL with island bridging using state-level boundaries.

This is a wrapper around make_with_islands.py that works with Natural Earth
state/province data instead of country data.
"""
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate Alaska STL with island bridging"
    )
    parser.add_argument("--dem", required=True, help="DEM file (e.g. nca_1km_smooth_aea.tif)")
    parser.add_argument("--ne-states", default="data/ne/ne_10m_admin_1_states_provinces.shp",
                       help="Natural Earth states/provinces shapefile")
    parser.add_argument("--output-dir", default="STLs_Alaska")
    ...
    try:
        # Process Alaska using the state name instead of country name
        # The process_country_with_islands function will work the same way
        process_country_with_islands(
            "Alaska",  # State name in Natural Earth states shapefile
            args.ne_states,  # Use states shapefile instead of countries
            dem_src,
            args.output_dir,
            args.step,
            target_faces,
