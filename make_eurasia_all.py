    ...
    for _, row in eurasia.iterrows():
        ...
        if geom.geom_type == "MultiPolygon":
            if country_name == "Azerbaijan":
                ...
                print(
                    f"  {country_name}: MultiPolygon detected, merged 2 polygons into single piece (bridged Bosphorus)"
                )
            elif country_name == "Denmark":
                # Merge Jutland peninsula + Zealand (Copenhagen) + Funen into single piece
                # Bridge the Great Belt and Øresund straits
                from shapely.geometry import MultiPolygon
                from shapely.ops import unary_union

                polys = sorted(geom.geoms, key=lambda p: p.area, reverse=True)

                # Take 3 largest: Jutland + Zealand + Funen
                # (excludes Bornholm and smaller islands)
                main_polys = MultiPolygon([polys[0], polys[1], polys[2]])

                # Buffer by ~10km to bridge straits, then merge
                buffered = main_polys.buffer(0.1)
                merged = unary_union(buffered)
                ...
                print(
                    f"  {country_name}: MultiPolygon detected, merged 3 polygons into single piece (Jutland + Zealand + Funen)"
                )
            elif country_name == "Malaysia":
                # Keep peninsula (131k km²) AND Borneo (196k km²)
                # Both are large landmasses needed for fitting with neighbors
                from shapely.geometry import MultiPolygon

                polys = sorted(geom.geoms, key=lambda p: p.area, reverse=True)
                keep_polys = [
                    p for p in polys if p.area > 0.5
                ]  # Keep both main landmasses (~1° threshold)
                geom = (
                    MultiPolygon(keep_polys) if len(keep_polys) > 1 else keep_polys[0]
                )
                print(
                    f"  {country_name}: MultiPolygon detected, keeping {len(keep_polys)} polygons (peninsula + Borneo)"
                )
            else:
        ...
        if country_name == "Kazakhstan" and geom.geom_type == "Polygon":
            ...
            if num_holes > 0:
                ...
                print(
                    f"  {country_name}: Removed {num_holes} interior ring(s) (Baikonur Cosmodrome lease)"
                )
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate ALL mainland Eurasia country STLs with QC outputs"
    )
    parser.add_argument(
        "--dem", required=True, help="Eurasia DEM file (eurasia_2km_smooth_aea.tif)"
    )
    parser.add_argument("--ne", required=True, help="Natural Earth admin0 shapefile")
    parser.add_argument(
        "--output-dir",
        required=True,
        help="Timestamped output directory (e.g., STLs_Eurasia_20260113_191624_33766c3)",
    )
    ...
    parser.add_argument(
        "--regions",
        nargs="+",
        choices=list(EURASIA_REGIONS.keys()),
    ...
    for region in regions_to_process:
        region_countries = {
            k: v for k, v in countries.items() if k in EURASIA_REGIONS[region]
        }
