...
def get_capital_xy_mm(transform, dem_shape, country_name, step, dem_crs=None):
    ...
    """
    print(
        f"    DEBUG: Looking for '{country_name}' in CAPITALS dict with {len(CAPITALS)} entries"
    )
    print(f"    DEBUG: Turkey in CAPITALS: {'Turkey' in CAPITALS}")
    info = CAPITALS.get(country_name)
    if info is None:
        print(f"    DEBUG: Capital info not found for '{country_name}'")
        return None
...
def cut_capital_star_hole(solid, capital_xy_mm):
    ...
    if capital_xy_mm is None:
        print("    DEBUG: capital_xy_mm is None, skipping star cut")
        return solid

    cx, cy = capital_xy_mm
    print(f"    DEBUG: Cutting star at ({cx:.1f}, {cy:.1f}) mm")
...
def process_country(
    ...
    # Captured AFTER all geometry-adding steps (clip, simplify, lakes, star)
    # but BEFORE mirror+scale. The driver uses these to write an authoritative
    # mesh CRS bbox into alignment.json so back-projection lands correctly
    # even when extras like the capital star push the mesh past its
    # vector-clip extent.
